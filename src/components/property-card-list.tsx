...
export function PropertyCardList({ property }: PropertyCardListProps) {
  ...
  return (
    <Card className="w-full overflow-hidden group border-none shadow-sm hover:shadow-xl transition-all duration-500 bg-card flex flex-col md:flex-row">
      <div className="relative md:w-2/5 aspect-[4/3] md:aspect-auto overflow-hidden">
        ...
        <div className="absolute bottom-4 left-4">
          <Badge className={`capitalize px-3 py-1 text-xs font-medium tracking-wide rounded-full ${displayStatus === 'Buy' || displayStatus === 'Rent' ? 'bg-[#2C2A26] text-white' : 'bg-green-600 text-white'}`}>
            {displayStatus}
          </Badge>
        </div>
      </div>
      <CardContent className="p-6 space-y-4 flex-1 flex flex-col justify-center">
        ...
        <div className="flex justify-between items-center pt-6 border-t border-border/50">
          ...
          <Button variant="default" className="rounded-full bg-[#2C2A26] hover:bg-[#433E38] text-white transition-all duration-300 px-8 dark:bg-white dark:text-[#2C2A26] dark:hover:bg-gray-200" asChild>
